...
SELECT region_id, COUNT(DISTINCT customer_id) AS num
FROM customer_nodes
GROUP BY region_id
...
SELECT ROUND(COUNT(txn_type)/COUNT(DISTINCT(customer_id)),2) AS deposit_counts, 
ROUND(AVG(txn_amount),2) AS avg_amount_deposit
FROM customer_transactions
...
count_withdrawal_by_month AS (
    SELECT
        customer_id,
        EXTRACT(MONTH FROM txn_date) AS month,
        COUNT(txn_type) AS count_withdrawal
    FROM
        customer_transactions
    WHERE
        txn_type = 'withdrawal'
...
count_purchase_by_month AS (
    SELECT
        customer_id,
        EXTRACT(MONTH FROM txn_date) AS month,
        COUNT(txn_type) AS count_purchase
    FROM
        customer_transactions
    WHERE
        txn_type = 'purchase'
...
SELECT
    count_deposit_by_month.customer_id,
    count_deposit_by_month.month,
    count_deposit_by_month.count_deposit,
    count_withdrawal_by_month.count_withdrawal,
    count_purchase_by_month.count_purchase
FROM
    count_deposit_by_month
LEFT JOIN
    count_withdrawal_by_month ON count_deposit_by_month.customer_id = count_withdrawal_by_month.customer_id
LEFT JOIN
    count_purchase_by_month ON count_deposit_by_month.customer_id = count_purchase_by_month.customer_id;
...
temp3 AS (
    SELECT
        *,
        MAX(txn_date) OVER (PARTITION BY customer_id, EXTRACT(MONTH FROM txn_date)) AS max_date
    FROM
        temp2
) -- Filter out the last transaction date for each user each month
SELECT
    customer_id,
